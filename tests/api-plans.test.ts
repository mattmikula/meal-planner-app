...
vi.mock("@/lib/plans/server", async () => {
  ...
  return {
    ...actual,
    fetchPlanForWeek: planMocks.fetchPlanForWeek
  };
});

vi.mock("@/lib/supabase/server", () => supabaseMocks);

import { GET as getPlan } from "@/app/api/plans/route";
...
describe("GET /api/plans", () => {
  it("returns unauthorized when auth fails", async () => {
    ...
    const response = await getPlan(
      new Request("https://localhost/api/plans?weekStart=2024-02-12")
    );
  ...
  it("returns 400 when weekStart is missing", async () => {
    ...
    const response = await getPlan(
      new Request("https://localhost/api/plans")
    );

    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "Week start is required." });
  });
  ...
  it("returns plan when authenticated", async () => {
    ...
    planMocks.fetchPlanForWeek.mockResolvedValue(samplePlan);
    ...
    expect(planMocks.fetchPlanForWeek).toHaveBeenCalledWith(
      supabase,
      "household-1",
      "user-1",
      "2024-02-12"
